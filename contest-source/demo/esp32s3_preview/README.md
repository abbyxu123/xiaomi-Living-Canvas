...
这是小米 openvela 大赛 Living Canvas 的独立 ESP32-S3 竖屏演示版。它不替代、也不修改 Gemini-S1/openvela 工程。
...
生成资源时，使用 `LC_ASSET_SOURCE_DIR` 指定包含 `circle.mp4` 和角色图片的目录，或分别设置 `LC_CIRCLE_VIDEO` 与 `LC_MENU_SOURCE`。固件工具可通过 `ARDUINO_CLI`、`MKSPIFFS_BIN` 和 `ESPTOOL_BIN` 指定；烧录前必须显式设置 `EXPECTED_ESP32_MAC`，防止写入错误设备。
